...
public class PostgresReplicationConnection extends JdbcConnection implements ReplicationConnection {
    ...
    private PostgresReplicationConnection(PostgresConnectorConfig config,
        ...
        super(addDefaultSettings(config.getJdbcConfig()), PostgresConnection.buildConnectionFactory(config.getDriver(), config.getProtocol(), config.getJdbcUrlSuffix()),
                "\"", "\"");
    ...
    private ServerInfo.ReplicationSlot getSlotInfo() throws SQLException, InterruptedException {
        try (PostgresConnection connection = new PostgresConnection(connectorConfig.getJdbcConfig(), PostgresConnection.CONNECTION_SLOT_INFO, connectorConfig.getDriver(),
